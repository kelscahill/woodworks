...
export default class Build extends Service {
     ...
     */
    async register() {
        await Promise.all(Object.entries(loaders).map(async ([key, loaderFactory]) => {
        ...
        await Promise.all(Object.entries(items).map(async ([key, itemFactory]) => {
        ...
        this.items.precss = this.app.isProduction
            ? this.items.minicss
            : this.items.style;
        await Promise.all(Object.entries(rules).map(async ([key, ruleFactory]) => {
