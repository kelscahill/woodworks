...
let BudBrowsersListCheck = class BudBrowsersListCheck extends Extension {
    constructor() {
        super(...arguments);
        this.items = [];
    }
    async beforeBuild() {
        if (!this.app.context.manifest.browserslist)
            return;
        ...
        this.app.context.manifest.browserslist?.development &&
            this.items.push(...this.app.context.manifest.browserslist.development);
        Array.isArray(this.app.context.manifest.browserslist) &&
            this.items.push(...this.app.context.manifest.browserslist);
        if (this.items.some(item => item.endsWith('@wordpress/browserslist-config'))) {
            try {
                await this.import('@wordpress/browserslist-config');
            }
            catch (e) {
                this.app.context.stdout.write(`
${c.red.bold(`\`@wordpress/browserslist-config\` is specified in \`package.json\` but it is not resolvable.`)}
...
`);
                ...
                if (this.app.extensions.has('@roots/bud-postcss')) {
                    const postcss = this.app.extensions.get('@roots/bud-postcss');
...
__decorate([
    bind
], BudBrowsersListCheck.prototype, "beforeBuild", null);
BudBrowsersListCheck = __decorate([
    label('@roots/bud-preset-wordpress/browserslist-check')
], BudBrowsersListCheck);
export default BudBrowsersListCheck;
