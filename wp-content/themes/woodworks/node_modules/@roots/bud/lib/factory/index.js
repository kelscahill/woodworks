 ...
 * @public
 */
export async function factory(overrides) {
    const context = await makeContext(overrides.dir ?? process.cwd());
    const project = await new Bud().lifecycle(mergeOptions(context, overrides));
    project
        .when(project.env.has('APP_PUBLIC_PATH') &&
        project.env.isString('APP_PUBLIC_PATH'), () => project.setPublicPath(project.env.get('APP_PUBLIC_PATH')))
        .log({
