...
let Cdn = class Cdn extends Extension {
    ...
    async register() {
        for (const cdnKey of this.sources.keys()) {
            if (this.app.project.has(`manifest.${cdnKey}`))
                this.enable();
        }
    }
    /**
...
Cdn = __decorate([
    ...
    options({
        allowedUris: [],
        cacheLocation: (app) => app.path('@storage', app.name, 'modules'),
        frozen: false,
        lockfileLocation: (app) => app.path('@storage', app.name, 'bud.lock'),
