 ...
 * @public
 */
const target = (command) => Object.keys(command.app.children).forEach(name => {
    !command.target.includes(name) && delete command.app.children[name];
});
export const config = async (command) => {
    !isUndefined(command.target) && target(command);
    if (!isUndefined(command.src)) {
        command.app.setPath('@src', command.src);
        Object.entries(command.app.children).map(([_name, child]) => child.setPath('@src', command.src));
    }
    if (!isUndefined(command.esm)) {
        command.app.esm.enable();
        Object.entries(command.app.children).map(([_name, child]) => child.esm.enable());
    }
    if (!isUndefined(command.immutable)) {
        command.app.cdn.freeze();
        Object.entries(command.app.children).map(([_name, child]) => child.cdn.freeze());
    }
    if (!isUndefined(command.dist)) {
        command.app.setPath('@dist', command.dist);
        Object.entries(command.app.children).map(([_name, child]) => child.setPath('@dist', command.dist));
    }
    if (!isUndefined(command.publicPath)) {
        command.app.setPublicPath(command.publicPath);
        Object.entries(command.app.children).map(([_name, child]) => child.setPublicPath(command.publicPath));
    }
    if (!isUndefined(command.manifest)) {
        command.app.hooks.on('feature.manifest', command.manifest);
        Object.entries(command.app.children).map(([_name, child]) => child.hooks.on('feature.manifest', command.manifest));
    }
    if (!isUndefined(command.cache)) {
        command.app.api.call(`persist`, command.cache);
        await Promise.all(Object.entries(command.app.children).map(async ([_name, child]) => {
    ...
    if (!isUndefined(command.hash)) {
        ...
        await Promise.all(Object.values(command.app.children).map(async (child) => {
