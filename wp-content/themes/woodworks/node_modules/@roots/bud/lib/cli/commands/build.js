...
const fallback = (test, value, fallback) => (isUndefined(test) ? fallback : value);
/**
 * Build command
 *
 * @public
...
export class BuildCommand extends BaseCommand {
    constructor() {
         ...
         * --dashboard
         */
        this.dashboard = Option.Boolean(`--dashboard`, undefined, {
            hidden: true,
        });
        this.debug = Option.Boolean(`--debug`, false, {
         ...
         * --dist
         */
        this.dist = Option.String(`--output,-o`, undefined, {
         ...
         * --html
         */
        this.html = Option.Boolean(`--html`, undefined, {
         ...
         * --mode
         */
        this.mode = Option.String(`--mode`, 'production', {
         ...
         * --src
         */
        this.src = Option.String(`--input,-i`, undefined, {
         ...
         * --indicator
         */
        this.indicator = Option.Boolean(`--indicator`, true, {
         ...
         * --modules
         */
        this.modules = Option.String(`--modules`, undefined, {
    ...
    async execute() {
        ...
        if (isUndefined(this.ci))
            this.context.args.ci = false;
        this.app = await factory({
            name: 'bud',
            mode: this.mode,
            context: this.context,
            seed: {
                'build.output.publicPath': fallback(this.publicPath, [() => this.publicPath], seed['build.output.publicPath']),
                'feature.inject': fallback(this.inject, [() => this.inject], seed['feature.inject']),
                'feature.log': fallback(this.log, [() => this.log], seed['feature.log']),
                'feature.manifest': fallback(this.manifest, [() => this.manifest], seed['feature.manifest']),
                'location.@src': fallback(this.src, [() => this.src], seed['location.@src']),
                'location.@dist': fallback(this.dist, [() => this.dist], seed['location.@dist']),
                'location.@storage': fallback(this.storage, [() => this.storage], seed['location.@storage']),
                'location.@modules': fallback(this.modules, [() => this.modules], seed['location.@modules']),
            },
        });
        await this.make();
